...
function filterProducts() {
    ...
    document.querySelectorAll('.product-card').forEach(card => {
        ...
        const price = parseFloat(card.querySelector('.price-val').innerText);
        const mType = (type === "all" || name.includes(type));
        const mPrice = (price >= min && price <= max);
        card.style.display = (mType && mPrice) ? "block" : "none";
    });
}
...
function openReservation(name, price, optionsString) {
    ...
    if(optionsString) {
        ...
        optBox.innerHTML = '<p style="font-size:0.7rem; font-weight:bold;">OPTIONS DISPO :</p>' + 
            arr.map(opt => `<div style="text-align:left;"><input type="checkbox" class="car-opt" value="${opt.trim()}"> <label>${opt.trim()}</label></div>`).join('');
...
async function confirmOrder() {
    ...
    if(!fName || !lName || !cin || !tel) return alert("Remplir tous les champs !");

    try {
        await db.collection("orders").add({
            prenom: fName, nom: lName, cin: cin, tel: tel,
